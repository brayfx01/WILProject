...
class Main:
    ...
    def run(self):
        ...
        result,self.optimalTanks = OptimalTanks(100000,100,100,0,self.difference,self.RTE, abs(totalNegative)).optimalTanks()
  
        # what happens if draing is greater than the remaining 
        print("AFTER")
        print(12 *100000 > abs(totalNegative))
        print("Should be 13 not 14 remember we only need enough energy for the totalNEgativ")
        ...
        print("BEFORE")
        self.optimaContainers = OptimalContainers(100000,100000,10,self.difference,self.optimalTanks,abs(totalNegative)).optimalContainers()
        print("after")
        
        for container in self.optimaContainers:
            print(container.cName,container.charge, "Charge")
            for tank in container.correspondingTanks:
                print(tank.tName)
        print("JUST GO OVER AND CHECK THE OPTIMAL TANKS THINK IT IS WRONG should be 13 not 14")
