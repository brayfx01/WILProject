...
class Graph:
    ...
    def GraphGeneratedLoadAndDifference(self):
        ...
        cursorGen.connect(
            "add", lambda sel: sel.annotation.set_text(f"Hour: {sel.target[0]}, Power: {sel.target[1]:.2f} kW") or sel.annotation.set_color(generatedLine[0].get_color())
        )
        ...
        cursorDiff.connect(
            "add", lambda sel: sel.annotation.set_text(f"Hour: {sel.target[0]}, Load: {sel.target[1]:.2f} kW")
