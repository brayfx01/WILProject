...
# the issue is that we charge full on the last so we do not exactly get what we want for the energy
class OptimalContainers():
    def __init__(self, maxCharge, minCharge, maxContainersPerSection, arrayOfEnergy, tanksArray,drain,RTE):
                ...
                if(currentContainer.remainingCharge >= abs(self.energy)):
                        ...
                        if(tankCapacity >= abs(self.energy)):  # this means we are done
                            ...
                            if(drain == True):
                                ...
                                # we drain by energy
                                self.totalDrain -= self.energy
                                tank.Charge(self.energy)
                                tankCapacity = tank.remainingCapacity()
                                # reduce the total drain that is remaining
                            ...
                            if(drain == True):
                               ...
                               # self.totalDrain = self.totalDrain - tank.remainingChargeCapacity()
                                # the tank is full
                                self.totalDrain -= tank.remainingCapacity()
                                tank.Charge(tank.remainingCapacity())
                                tankCapacity = tank.remainingCapacity()
                        
                            # checking for duplicate tank
                            if(self.duplicateTank(currentContainer, tank) == False):
                                currentContainer.correspondingTanks.append(tank)
                            #this tank is empty
                            self.empty = True
                ...
                # we will need to use multiple containers
                    ...
                    if(tankCapacity >= abs(self.energy)):
                        ...
                        if(drain == True):
                            ...
                            self.totalDrain -= currentContainer.remainingCharge
                        ...
                        if(tankCapacity >= currentContainer.remainingCharge):
                            ...
                            if(currentContainer.cName == "Container: 2"):
                                print("FOUND")
                                for t in currentContainer.correspondingTanks:
                                    print(t.tName)
                                    print(tank.tName)
                                for c in containerArray:
                                    if(c.cName == "Container: 2"):
                                        for t in  c.correspondingTanks:
                                            print(t.tName)
                        ...
                        # tank is least
                            ...
                            if(tankCapacity <= currentContainer.charge):
                                ...
                                if(drain == True):
                                    ...
                                    self.totalDrain -= tankCapacity
                                    # tank is full
                                    tank.Charge(tankCapacity)
                                    tankCapacity = tank.remainingCapacity()
                             
                          
                                       # IFF not duplicate then add
                                if(self.duplicateTank(currentContainer, tank) == False):
                                    currentContainer.correspondingTanks.append(tank)
                        """
                        if(currentContainer.cName == "Container: 2"):
    ...
    def optimalContainers(self):
        ...
        for energy in self.energyArray:
            ...
            energy = energy/(self.RTE.efficency/100)
            self.energy = energy
            ...
            while(self.energy != 0 or self.energy > 0 and self.totalDrain > 0 ):
                ...
                for tank in tempTankArray:
                    ...
                    if(tankCapacity >= abs(self.energy)):
                        ...
                        while self.energy != 0:
                            ...
                            # next is if our max charge is less than the energy
                                ...
                                if(charge == True):
                                    self.totalDrain -= self.maxCharge
                    ...
                    # this means we will need to go over multiple tanks
                        ...
                        if(self.maxCharge >= abs(self.energy)):
                            ...
                            # if false then we need a new container else we just connect a previous one
                            if(multipleConnections == False):
                                cont = container("Section: " + str(currentSection + 1), "Container: " + str(
                            ...
                            # append this tank to the container
                            cont.correspondingTanks.append(tank)
