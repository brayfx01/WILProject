...
class systemInitalization():
    def __init__(self, generated,load,configFile):
        ...
        for i in range(numSections):
            ...
            self.cells.append(Section(sectionName, numberOfContainers, self.getCriticalInfo("Container On Off Efficency",0),self.tanks,configFile))

        #now we want to go and get the number of containers in each section
         #this is getting the number of containers in a section
    def generateSurplus(self,generatedColumnName, loadColumnName):
        difference = self.generated[generatedColumnName] - self.load[loadColumnName]
        difference = difference.dropna()
        return difference
        

    def getCriticalInfo(self,target,operation):
        # Read the text file
        targetNumber = 0
        targetText = ""
            ...
            for line in file:
                ...
                if  target in line:
                    ...
                    if(operation == 0):
                        ...
                        targetNumber = int(number[0])
                    else:
                      
                        targetText = parts[1].strip()
                        return targetText
...
#now we want to go and get the number of containers in each section
   #this is getting the number of containers in a section
    ...
    def getTanks(self):
        return self.tanks
    def getRTE(self):
        self.efficency = self.getCriticalInfo("Round Trip Efficency",0)
