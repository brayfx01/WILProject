...
class Graphs:
    ...
    def closeWindow(self,operation):
        if(operation == 0):
            ...
            self.finished = True
            # choosing a place to save the configuration file and the tanks and contaienrs
            file_path = filedialog.asksaveasfilename(defaultextension=".txt", filetypes=[("Text Files", "*.txt")])
            self.savedFile = file_path
            # if there is a file_path then we exit otherwise there was none chosen
            if file_path:
              self.root.quit()
        else:
            quit()
    ...
    def createCanvas(self,fig1,fig2,fig3, fig4):
        imageFrame = tk.Frame(self.root,  borderwidth=1,relief="solid")
        imageFrame.grid(row = 0, column = 0)
         # Create Canvas widgets for Matplotlib figures
        canvas1 = FigureCanvasTkAgg(fig1, master=imageFrame)
        canvas2 = FigureCanvasTkAgg(fig2, master=imageFrame)
        canvas3 = FigureCanvasTkAgg(fig3, master=imageFrame)
        canvas4 = FigureCanvasTkAgg(fig4, master=imageFrame)
        ...
        canvas1_widget.grid(row=1, column=0,padx= 5)
        canvas2_widget.grid(row=1, column=1,padx= 5)
        canvas3_widget.grid(row=3, column=0,padx= 5)
        canvas4_widget.grid(row=3, column=1,padx= 5)
        # creating frames to house  buttons
        figOneButtonFrame = tk.Frame(imageFrame)
        figOneButtonFrame.grid(row = 2, column= 0, sticky= "NSEW", padx = 10)

        figTwoButtonFrame = tk.Frame(imageFrame)
        figTwoButtonFrame.grid(row = 2, column= 1, sticky= "nsew", padx = 10)

        figThreeButtonFrame = tk.Frame(imageFrame)
        figThreeButtonFrame.grid(row = 4, column= 0, sticky= "nsew", padx = 10)

        figFourButtonFrame = tk.Frame(imageFrame)
        ...
        saveAndContinue = ttk.Button(bottomButtonFrame, text = "SAVE AND EXIT", command=lambda: self.closeWindow(1))
    ...
    def graph(self):
        ...
        time = 0
        timestamps = []
        # we are getting the y axis in this case time in hours
        for element in self.data:
            time = time +  5
            # append this in hours
            timestamps.append(time/60)
        time = 0 
        tankTime = []
        for element in self.optimalTanks:
            time  += 5
            tankTime.append(time/60)
        ...
        for element in self.optimalContainers:
            time += 5
            contTime.append(time/60)
        time =0
        socTime = []

        for element in self.totalSoc:
            time += 5
            socTime.append(time/60)
       ...
       # Run the Tkinter main loop
        ...
        if(self.root.winfo_exists == True and self.finished == True):
            ...
            self.root.destroy()
        return self.finished,self.savedFile