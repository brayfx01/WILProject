...
class Review:
    ...
    def update_text_widget(self,configFileName,text_widget):
        try:
            ...
            pass
    # changes the load or generated widget if they are changed by checking if new data equals current data
    def update_csv_widget(self,csvFileName,csv_widget):
        ...
        try:      
            with open(csvFileName, "r", newline="") as f:
                ...
                new_csv_data = f.read()
                if new_csv_data != self.genFile or self.loadFile:
    ...
    def ChangeConfig(self,  textWidget,textHeaderLabel):
        file_path = filedialog.askopenfilename(filetypes=[("Text Files", "*.txt")])
        # only update these if we chose something
        if(file_path):
            self.configHeader = os.path.basename(file_path)
            textHeaderLabel.config(text = "Configruation File: "  + self.configHeader)
  
            self.textFile = file_path
            self.update_text_widget(self.textFile, textWidget)

    def ChangeCsv(self,textFileName, csvWidget, headerLabel):
        file_path = filedialog.askopenfilename(filetypes=[("CSV Files", "*.csv")])
        print("FILE:PATH", file_path)
        if file_path:
            file_name = os.path.abspath(file_path)
            #set the dataset name to this
            self.csvName = os.path.basename(file_name)
            #textFileName.config(text=f"File Name: {file_name}") 
          
            headerLabel.config(text = "Dataset: " + self.csvName )
   
            self.csvFile = file_name
   
            self.update_csv_widget(os.path.abspath(file_path), csvWidget)
    ...
    def createWindow(self):
        ...
        headerName = "Configruation File: " + os.path.basename(self.textFile)
        ...
        genheaderName = "Dataset: " + os.path.basename(self.genFile) # we just want to display the name of the file
        genHeaderLabel = tk.Label(csv_frame,text = genheaderName)
        genHeaderLabel.grid(row = 1 , column=0, sticky="nsew")
        
        loadHeaderName = "Dataset: "  + os.path.basename(self.loadFile)
        ...
        csv_widget.config(yscrollcommand=csv_scrollbar.set)
                ...
                # Create a Scrollbar widget for the CSV widget
        loadScrollBar = tk.Scrollbar(loadFrame, command= loadWidget.yview, orient=tk.VERTICAL)
        loadScrollBar.grid(row=2, column=1, sticky="ns")  # Position it to the right
        
        # moves the scroll bar
        loadWidget.config(yscrollcommand=loadScrollBar.set)
        ...
        changeButtonForConfig = ttk.Button(button_frame_text, text="CHANGE" ,command = lambda: self.ChangeConfig(text_widget,textheaderLabel))
        ...
        changeButtonForCsv = ttk.Button(button_frame_csv, text="CHANGE", command = lambda: self.ChangeCsv(self.genFile,csv_widget,genHeaderLabel))
        ...
        changeButtonForLoad = ttk.Button(loadButtonFrame, text="CHANGE", command = lambda: self.ChangeCsv(self.loadFile,loadWidget,loadHeaderLabel))
